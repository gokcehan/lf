...
When this option is enabled, filenames containing right to left or Arabic characters are reordered to visual order for display, along with Arabic presentation forms and mandatory ligatures.
Only the displayed text is affected, filesystem paths always retain their original logical order.
