...
Register-ArgumentCompleter -Native -CommandName 'lf' -ScriptBlock {
    ...
    $completions = @(
        ...
        [CompletionResult]::new('-last-dir', '-last-dir', [CompletionResultType]::ParameterName, 'print the last dir to stdout on exit (to use for cd)')
        ...
        [CompletionResult]::new('-selection', '-selection', [CompletionResultType]::ParameterName, 'print the selected files to stdout on open (to use as open file dialog)')
