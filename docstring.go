...
When this option is enabled, the items in LS_COLORS are considered as globs,
otherwise they are only considered to be the extension of a filename. With
globbing, '*' matches any sequence, '?' matches any character, and '[...]'
or '[^...] matches character sets or ranges. Otherwise, these characters are
interpreted as they are.

    globicon       bool      (default off)

When this option is enabled, the items in LF_ICONS are considered as globs,
otherwise they are only considered to be the extension of a filename. See
'globcolor' for an explanation of globs.

    globsearch     bool      (default off)

When this option is enabled, search command patterns are considered as
globs, otherwise they are literals. See 'globcolor' for an explanation of
globs.
...
directory, and '%f' as the file name. Home folder is shown as '~' in the
working directory expansion. Directory names are automatically shortened to
a single character starting from the left most parent when the prompt does
not fit to the screen.
...
First, you can configure 8 basic colors used by your terminal and lf should
pick up those colors automatically. Depending on your terminal, you should
be able to select your colors from a 24-bit palette. This is the recommended
approach as colors used by other programs will also match each other.
...
best results. lf can also be configured to ignore your terminal theme and
