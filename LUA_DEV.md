...
- A Lua state pool, distributing Lua states to different goroutine when needed,
makes running Lua code asynchronously possible.
...
And each value in this table must also be a table.
...
The registry table structure helps lf execute message action on different Lua
state with no problem.

When calling a message, lf will try to locate a message action with 4 components:

- Source name, this is the path to plugin entrance script that provides this message.
- Registry key, used for indexing tables returned by plugin scripts.

  This is also registry type.
- Message name.
...
Before a Lua state is used for message execution, it needs to be initialized by
running all plugin scripts, functions and tables yielded during initialization is
local to this Lua state.

So for latter message lookup, tables returned by plugin scripts are saved in map
in a global struct, with pointer to corresponding Lua state as key.
...
is fetched from Lua state pool.
...
action function found in message entry table, and sometimes returns default
...
- `command`, adds new command to lf.

  Entry type: `string`, `function`, `table`

  Message variant:

  - `action`, a function to run when calling this command, can be a string or a function
  - `completion`, a function that returns a list of `CompMatch` and matched string
- `event_hook`, adds callback function for lf event like `on-init`, `on-load`.
...
- `key_map`, defines new key map and uses a slightly diferent registry table structure.
  ...
  Above table can be indexed as `key_map`, mapping mode, target key, key map entry.

  Here, key map entry is just like message entry used in other registry type.
...
- `previewer`, adds new preview action.
...
- `sorting_method`, adds new sorting method to lf.
...
- `ui_formatter`
  ...
  - `action`, a function, its argument and return type differ by UI element type.
...
- `ui_style`
  ...
  borderfmt, copyfmt, cutfmt, menufmt, menuheaderfmt, menuselectfmt, selectfmt, visual

  Message names in this registry table are names of those options but without
  that `fmt` suffix.
